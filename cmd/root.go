...
func NewRootCommand() *cobra.Command {
	...
	client.Flags().Duration(InitialRetryDelayFlag, 1*time.Second, "Initial retry delay")
	client.Flags().Duration(MaxRetryDelayFlag, 30*time.Second, "Maximum retry delay")
	client.Flags().Float64(RetryMultiplierFlag, 2.0, "Retry delay multiplier for exponential backoff")
	...
	client.Flags().Uint32(CircuitBreakerMaxRequestsFlag, 5, "Circuit breaker: maximum concurrent requests allowed in half-open state (recovery testing)")
	client.Flags().Duration(CircuitBreakerIntervalFlag, 1*time.Minute, "Circuit breaker: sliding window duration for counting failures before resetting")
	client.Flags().Duration(CircuitBreakerTimeoutFlag, 30*time.Second, "Circuit breaker: wait time in open state (failing) before attempting recovery via half-open state")
	client.Flags().Uint32(CircuitBreakerConsecutiveFailuresFlag, 3, "Circuit breaker: consecutive failure threshold to trip from closed (normal) to open (failing)")
